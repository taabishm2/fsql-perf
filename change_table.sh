...
mysql -u username -p password -e "ALTER DATABASE ${database_name} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
...
tables=$(mysql -u username -p password -Nse "SHOW TABLES" ${database_name})
for table in $tables; do
  sql_statement="ALTER TABLE ${table} ENGINE=${engine_name};"
  echo "Changing engine for table ${table}..."
  mysql -u username -p password -e "${sql_statement}" ${database_name}
done
